document.addEventListener('DOMContentLoaded', () => {
  ...
  const clickOkButton = document.getElementById('clickOkButton'); // Add this line
    ...
    try {
      ...
      const results = await chrome.scripting.executeScript({
        ...
        args: [sentences, {
          ...
          clickOkButton: clickOkButton ? clickOkButton.checked : false // Safe check
        }]
      });
      
      const { fieldsFilled, checkboxesChecked, dropdownsSet, okButtonClicked } = results[0].result;
      ...
      if (okButtonClicked) message += `, clicked OK button`;
...
function automateForm(sentences, options) {
  ...
  // 4. Click OK button with built-in delay
  let okButtonClicked = false;
  if (options.clickOkButton) {
      const okButton = document.getElementById('idButtonOk');
      if (okButton && !okButton.disabled) {
          // Built-in 1 second delay before clicking OK
          setTimeout(() => {
              okButton.click();
